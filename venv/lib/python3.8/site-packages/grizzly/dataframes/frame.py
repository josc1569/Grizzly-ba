...
from grizzly.pdOperations import pandasOperations,DuckDBOperations, ApacheArrowOperations
import inspect
...
class DataFrame(object):
  ...
  def __getattr__(self, name):
    ...
    def handler(**kwargs):
      return self._inner_getattr(name, **kwargs)
    return handler

    '''ToDo: get the try except block running
    try:
  ...
  def _inner_getattr(self, name, **kwargs):
    mode = 'pandas'
    if 'mode' in kwargs.keys(): mode = kwargs['mode']
    if (mode == 'pandas'):
      return self.exec_pandas(name, **kwargs)
    elif (mode == 'duckdb'):
      return self.exec_duckdb(name,**kwargs)
    elif (mode == 'apachearrow'):
      return self.exec_apachearrow(name, **kwargs)

  def exec_pandas(self, name, **kwargs):
    pdf = self.sql_to_dataframe()
    func = getattr(pandasOperations, '__' + name + '__')
    pdf = func(pdf, **kwargs)
    return pdf

  def exec_duckdb(self, name, **kwargs):
    import duckdb
    conn = duckdb.connect()
    pdf = self.sql_to_dataframe()
    rel = conn.from_df(pdf)
    func = getattr(DuckDBOperations, '__' + name + '__')
    res = func(rel, **kwargs)
    return res

  def exec_apachearrow(self, name, **kwargs):
    import pyarrow as pa
    pdf = self.sql_to_dataframe()
    table = pa.Table.from_pandas(pdf)
    func = getattr(ApacheArrowOperations, '__' + name + '__')
    res = func(table, **kwargs)
    return res
  ...
  def sql_to_dataframe(self):
    con = grizzly.get_connection(self)
    df_str = self.generateQuery()
    return pandas.read_sql(df_str, con)
