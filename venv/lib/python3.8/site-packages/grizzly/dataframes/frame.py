...
from grizzly.pdOperations import pandasOperation
import inspect
...
class DataFrame(object):
  ...
  def __getattr__(self, name):
    ...
    if name in pandasOperation.__members__:
      def handler(**kwargs):
        return self._inner_getattr(self,name, **kwargs)
      return handler
    else:
      return ColRef(name, self)
  ...
  def _inner_getattr(self, name, **kwargs):
    ...
    pdfunc = getattr(pandasOperation, '__' + name + '__')
    pdf = pdfunc(pdf, **kwargs)
    return pdf

  def exec_pandas(self, name, **kwargs):
      pdf = self.sql_to_dataframe()
      func = getattr(pandasOperation, '__' + name + '__')
      pdf = func(pdf, **kwargs)
      return pdf'''
