...
class DuckDBOperations(Enum):
    ...
    def __merge__(self, **kwargs):
        ...
        if (len(kwargs['cond']) == 1):
            cond = 'pdf1.' + kwargs['cond'][0] + ' = pdf2.' + kwargs['cond'][0]
        elif (len(kwargs['cond']) == 2):
            cond = 'pdf1.' + kwargs['cond'][0] + ' = pdf2.' + kwargs['cond'][1]
        else:
            raise TypeError

        res = con.execute("SELECT * FROM pdf1 " + kwargs['how'] + " pdf2 ON " + cond).fetchdf()
        return res
...
class ApacheArrowOperations(Enum):
    ...
    def ___join__(self, **kwargs):

        if isinstance(kwargs['table'], grizzly.dataframes.frame.DataFrame):
            pdf = kwargs['table'].sql_to_dataframe()
            table = pyarrow.Table.from_pandas(pdf)
        else:
            table = kwargs['table']
        res = join(self, tables, on=['globaleventid'])
        return res
    def __concat_tables__(self, **kwargs):
        ...
        promote = False
        memory_pool = None
        res = pyarrow.concat_tables(tables=[self, tables], promote=promote, memory_pool=memory_pool)
        return res
